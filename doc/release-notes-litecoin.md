Scorcoin Core version *0.15.1* is now available from:

  <https://download.scorcoin.org/scorcoin-0.15.1/>
...
A number of changes to the way Scorcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 sco per kB (0.00001 SCO). This is to prepare for dropping the minimum transaction fee to 0.1 sco per kB (0.0001 SCO) in the next release.
