...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 99135
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 99125
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 199135
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 199125
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 99135
	%{_sbindir}/semanage port -d -p tcp 99125
	%{_sbindir}/semanage port -d -p tcp 199135
	%{_sbindir}/semanage port -d -p tcp 199125
	for selinuxvariant in %{selinux_variants}; do
